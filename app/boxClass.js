...
function tool(db) {
  ...
  const cls = {
    destruct: function (opt = {}) {
      ...
      hide_log || (logg.t(`probe destruct  ${db.enabled ? 'enabled ' : 'disabled'} [${String(db.id).padStart(6, ' ')}]`));
    },
    construct: async function () {

      logg.t(`probe construct ${db.enabled ? 'enabled ' : 'disabled'} [${String(db.id).padStart(6, ' ')}]`);
